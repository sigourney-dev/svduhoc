...
export const DetailNewsScreen = (props: any) => {
  ...
  const {postDetailError, postDetailResult, isLoading} = useSelector((store: any) => store.post);
  const {detailAbroadResult, detailAbroadError} = useSelector((store: any) => store.abroad);
  ...
  useEffect(() => {
    if (!detail) {
     if (type === 'ABROAD') {
      dispatch(postActions.getDetailAbroadRequest({
        id: idPost,
      }));
     } else {
